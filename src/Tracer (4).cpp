...
glm::vec3 Tracer::ModelFong(Ray ray, Triangle pol, glm::vec3 baryCoord, glm::vec3 color)
{
    ...
    /*while(1)
    {
        ...
        //std::cout << tpoint.x << ' ' << tpoint.y << ' ' << tpoint.z << std::endl;
        //std::cout << scene.min_point.x << ' ' << scene.min_point.y << ' ' << scene.min_point.z << std::endl;
        //std::cout << scene.max_point.x << ' ' << scene.max_point.y << ' ' << scene.max_point.z << std::endl;
        ...
        //std::cout << RezultColor.x << ' ' << RezultColor.y << ' ' << RezultColor.z << std::endl;
        //RezultColor = RezultColor + (Color - RezultColor) * alfa;
        RezultColor = Color * alfa + RezultColor * (1 - alfa);
        //std::cout << RezultColor.x << ' ' << RezultColor.y << ' ' << RezultColor.z << std::endl;
        len += glm::length(scene.step / 100.0f);
    }*/
...
void Tracer::InitAntena()
{
    ...
    float max_len = max(glm::length(antena.getAntenaPosition() - scene.min_point), glm::length(scene.max_point - antena.getAntenaPosition()));
    for(int i = 0; i < num_ray; i++) {
        ...
        while(1)
        {
            ...
            if(scene.anten_signal[floor(ind.x)][floor(ind.y)][floor(ind.z)] < 1 - ((len * len)/(max_len * max_len))) {
                ...
                std::cout << 1 - ((len * len)/(max_len * max_len)) << std::endl;
            }
            len += glm::length(scene.step / 800.0f);
        }
        /*for(int i = 1; i < 99; i++) {
            for(int j = 1;j < 99; j++) {
                for(int q = 1; q < 99; q++) {
